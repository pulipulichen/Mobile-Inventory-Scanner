...
「掃描」分頁可以在「相機」與「刷槍」兩種輸入方式之間切換，選擇會保存在目前瀏覽器。
...
2. 按「開始掃描」。
3. 第一次使用時，請允許瀏覽器使用相機。
4. 使用手機後置鏡頭對準 QR Code。
5. 將 QR Code 保持在畫面中，直到頁面顯示已辨識。
6. 可一次讓畫面中出現多個 QR Code。
...
如果即時相機無法使用，請先切換成「相機」輸入方式：

1. 在「掃描」分頁按「拍攝 QR Code」。
2. 使用手機後置鏡頭拍攝一張照片。
3. 拍照完成後，系統會在瀏覽器本機辨識照片中的所有 QR Code。
4. 若沒有辨識到 QR Code，請調整距離、光線或照片角度後重新拍攝。

### 5.6 讀取既有相片

1. 按「讀取相片」。
2. 從手機照片或檔案選擇器選取一張圖片。
3. 系統會辨識圖片中的所有 QR Code。
4. 若照片無法讀取或沒有 QR Code，請重新選擇清楚的圖片。
...
- 增加光線，避免反光。
- 讓 QR Code 完整出現在照片或預覽範圍內。
- 調整手機與 QR Code 的距離。
- 確認 QR Code 沒有被裁切、折彎或嚴重模糊。
...
4. Google Sheet 中的 ID 與 QR Code 內容完全一致。
